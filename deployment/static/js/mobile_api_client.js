...
class MobileAPIClient {
    constructor() {
        // Production uses single CloudFront-backed domain for all API traffic
        this.apiEndpoint = 'https://www.ieltsaiprep.com';
        this.websocketEndpoint = 'wss://www.ieltsaiprep.com';
        
        // Regional endpoints (reserved for future multi-region deployment)
        this.regionalEndpoints = {
            'us-east-1': 'https://www.ieltsaiprep.com',
            'eu-west-1': 'https://www.ieltsaiprep.com',
            'ap-southeast-1': 'https://www.ieltsaiprep.com'
        };
        this.websocketEndpoints = {
            'us-east-1': 'wss://www.ieltsaiprep.com',
            'eu-west-1': 'wss://www.ieltsaiprep.com',
            'ap-southeast-1': 'wss://www.ieltsaiprep.com'
        };
    ...
    getRegionalEndpoint() {
        // All regions route to same CloudFront domain (future: actual regional endpoints)
    ...
    async makeAPICall(endpoint, method = 'POST', data = null, options = {}) {
        ...
        // All traffic routes through single CloudFront distribution
        const baseUrl = this.apiEndpoint;
        
        const fullUrl = `${baseUrl}${endpoint}`;
        
        // Extended timeout for speech assessments
        const requestTimeout = endpoint.includes('/speaking/') || endpoint.includes('/nova-sonic/') ? 20000 : timeout;
    ...
    async startNovaSonicStream(audioData, assessmentType) {
        return new Promise((resolve, reject) => {
            try {
                // Connect to CloudFront WebSocket endpoint
                ...
                ws.onopen = () => {
                    console.log('Speech assessment WebSocket connected');
    ...
    async showLatencyNotification() {
        // Show user-friendly notification about speech assessment processing
        const message = 'Connecting to our AI speech assessment service. This may take a few moments for optimal processing.';
